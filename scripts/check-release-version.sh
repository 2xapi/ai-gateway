...
index_version=$(sed -n 's/.*id="appVersionBadge"[^>]*>v\([^<]*\)<.*/\1/p' frontend/index.html | head -1)
...
if [[ -n "$index_version" && "$tag_version" != "$index_version" ]]; then
  echo "version mismatch: tag=$tag_version frontend/index.html=$index_version" >&2
  exit 1
fi
