...
//!
//! 模块拆分（行为零变化，仅按职责搬移）：
//! - [`forward`]：client 构建、加速发送核心（send_with_accel/407 换线重试）、流式读取与 HTML 上游判别；
//! - [`passthrough`]：官方通道透传（chatgpt.com 后端 + JWT 账号解析）；
//! - [`usage`]：用量台账（请求行/响应体/流式 usage 落盘）；
//! - [`accel`]：加速线路装配（线路选择、per-Key 凭证确保、407 判别与降级记账）。
...
use accel::{accel_plan, ensure_line_cred};
use forward::{
    build_client, build_line_client, is_html_upstream, next_stream_chunk, proxy_auth_error,
    read_body_timed, send_with_accel, stream_body_with_usage,
};
use passthrough::passthrough_official;
use usage::{flush_stream_usage, usage_log, usage_log_response};

// 仅供本模块单测通过 `use super::*` 引用(非测试构建不导入)
#[cfg(test)]
use crate::acclines::AccLine;
#[cfg(test)]
use forward::SendMeta;
#[cfg(test)]
use passthrough::jwt_chatgpt_account_id;
#[cfg(test)]
use usage::collect_stream_usage;
